...
public class Electro extends CustomCard {

    public static final String ID = ModHelper.makePath("Electro");
    private static final CardStrings CARD_STRINGS = CardCrawlGame.languagePack.getCardStrings(ID);
    private static final String NAME = CARD_STRINGS.NAME;
    private static final String IMG_PATH = "GenshinModResources/img/cards/element/Electro.png";
    ...
    public Electro(AbstractMonster m) {
        this();
        this.targetMonster = m;
    }

    public Electro(AbstractMonster m, int magicNumber) {
        this();
        this.targetMonster = m;
        this.magicNumber = magicNumber;
    }

    public Electro(AbstractMonster m, int magicNumber, boolean applyToAllEnemies) {
        this();
        this.targetMonster = m;
        this.magicNumber = magicNumber;
    ...
    public void onChoseThisOption() {
        ...
        if (applyToAllEnemies) {
            for (AbstractMonster monster : AbstractDungeon.getCurrRoom().monsters.monsters) {
                applyElectroPowerWithReaction(monster, p, magicNumber);
            }
        } else if (targetMonster != null) {
            if (magicNumber > 0) {
                applyElectroPowerWithReaction(targetMonster, p, magicNumber);
            } else {
                applyElectroPower(targetMonster, p);
            }
        }
    }

    private void applyElectroPowerWithReaction(AbstractCreature target, AbstractPlayer source, int amount) {
        this.addToBot(new ApplyElementalPowerAction(target,source,ModHelper.makePath("ElectroPower"),amount));
        this.addToBot(new ElementalReactionAction(target, source));
    }

    private void applyElectroPower(AbstractCreature target, AbstractPlayer source) {
